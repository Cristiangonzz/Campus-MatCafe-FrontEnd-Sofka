...
const learner$ = this.delegateAdmin.getLearnerByEmailUseCaseProvaider.useFactory(this.adminService).execute(learnerEmail).pipe(map((learner) => ({ learnerId: learner._id })));
const route$ = this.delegateRoute.getRouteByNameUseCaseProvaider.useFactory(this.routeService).execute(routeName).pipe(map((route) => ({ routeId: route._id })));
...
forkJoin([learner$, route$]).subscribe(([learner, route]) => {
    ...
    const subscribe: ISuscribeRoute  = {
     ...
     this.delegateLearner.subscribeRouteCaseProvaider.useFactory(this.learnerService).execute(subscribe).subscribe();
